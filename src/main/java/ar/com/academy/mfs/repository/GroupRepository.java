...
public interface GroupRepository extends JpaRepository<Group, Integer> {
	...
	@Query(value = "select supervisor_id from msf.group where group_number = ?1 and active = true limit 1", nativeQuery = true)
	int findLeaderByGroupNumber(int group_number);
	
	@Query(value = "select * from msf.group where supervisor_id = ?1 and active = true", nativeQuery = true)
